...
async def get_token(request: TokenRequest):
    ...
    if request.client_id not in VALID_CLIENTS or VALID_CLIENTS[request.client_id] != request.client_secret:
...
async def mcp_health():
    ...
    try:
        ...
        if not hasattr(global_mcp_instance, "_mcp_server") or not hasattr(global_mcp_instance, "_allow_anonymous"):
             ...
             if not hasattr(global_mcp_instance, "_allow_anonymous"):
                 ...
                 global_mcp_instance._server_name = "Enterprise MCP ASGI Server (Initialized)"
                 logger.warning("Manually initialized Enterprise MCP config for health check")
             else:
                 return {
                     "status": "unhealthy",
        ...
        return {
            ...
            "allow_anonymous": getattr(global_mcp_instance, "_allow_anonymous", "Unknown"),
