...
# Default JWT configuration from environment variables
JWT_SECRET_KEY = os.getenv("JWT_SECRET_KEY", secrets.token_hex(32))
JWT_ALGORITHM = os.getenv("JWT_ALGORITHM", "HS256")
JWT_ACCESS_TOKEN_EXPIRE_MINUTES = int(os.getenv("JWT_ACCESS_TOKEN_EXPIRE_MINUTES", "120"))  # 2 hours
...
def check_scope_permission(required_permission: str, granted_scopes_str: Optional[str]) -> bool:
    ...
    if len(parts) != 2:
        logger.warning(f"Invalid permission format: '{required_permission}' - should be 'resource:action'")
    ...
    if resource_wildcard in granted_scopes:
        logger.debug(f"Permission '{required_permission}' granted - resource wildcard match ({resource_wildcard})")
        return True
    ...
    if action_wildcard in granted_scopes:
        logger.debug(f"Permission '{required_permission}' granted - action wildcard match ({action_wildcard})")
        return True

    logger.warning(f"Permission '{required_permission}' denied - no matching scope found in {granted_scopes}")
    return False
...
class AuthService:
    ...
    def __get_pydantic_json_schema__(cls, _core_schema, handler):
        ...
        """
        return {"type": "object", "title": "AuthService", "description": "Authentication and authorization service instance"}
    ...
    def decode_token(self, token: str) -> Dict:
        ...
        try:
            ...
            logger.debug(f"JWT decoding failed for token (might be OAuth token): {token[:10]}... Error: {e}")
    ...
    async def authenticate_user(self, username: str, password: str) -> Optional[Dict[str, Any]]:
        ...
        if not user.get('password_hash') or not self.verify_password(password, user['password_hash']):
            return None
    ...
    async def initialize_roles_and_permissions(self):
        ...
        # Role definitions
        roles_to_create = [
            {"name": "admin", "description": "Full administrative access with all permissions"},
            {"name": "tool_creator", "description": "Can create and manage tools"},
            {"name": "tool_user", "description": "Can execute tools"},
            {"name": "readonly", "description": "Read-only access to system information"},
            {"name": "auditor", "description": "Can view audit logs"},
            {"name": "api_gateway", "description": "Internal role for API Gateway communication"}
        ]
        ...
        permissions_to_create = [
            ...
            {"name": "tool:read", "description": "Read tool definitions and list tools"},
            ...
            {"name": "role:update", "description": "Update roles and assign permissions"},
            {"name": "role:delete", "description": "Delete roles"},
            {"name": "role:assign", "description": "Assign roles to users"},
            {"name": "log:read", "description": "Read audit logs"},
            {"name": "gateway:configure", "description": "Configure gateway settings (e.g., allowed imports)"}
        ]
        ...
        role_permissions = {
            ...
            "tool_creator": ["tool:create", "tool:read", "tool:update", "tool:delete", "tool:backup", "tool:restore"],
        ...
        async with self.db.pool.acquire() as conn:
            async with conn.transaction():
                ...
                if existing_roles and existing_roles[0]['count'] > 0:
                    logger.info(f"Found {existing_roles[0]['count']} existing roles. Checking for admin role consistency.")
                    
                    # Check if the admin role exists and has the right description
                    admin_role = await conn.fetchrow("SELECT id, description FROM roles WHERE name = 'admin'")
                    if admin_role:
                        await conn.execute(
                ...
                for perm in permissions_to_create:
                    ...
                    if not perm_id: # If failure in returning id, get existing ID
                        perm_id = await conn.fetchval("SELECT id FROM permissions WHERE name = $1", perm["name"])
                ...
                for role in roles_to_create:
                    ...
                    if not role_id: # If failure in returning id, get existing ID
                        role_id = await conn.fetchval("SELECT id FROM roles WHERE name = $1", role["name"])
        ...
        if payload.get("_jwt_decode_failed"): # Check if JWT decode failed
            ...
            if user_id:
                ...
                if user is None:
                    ...
                    raise credentials_exception
                # Return user info
                return {"id": user_id, "username": user["username"], "type": "user", "scope": scope}
            elif client_id:
                # This is a client credentials token
                # Return client info (no specific user)
                return {"id": None, "client_id": client_id, "type": "client", "scope": scope}
            else:
                logger.error(f"Valid OAuth token {token[:10]}... has neither user_id nor client_id.")
                raise credentials_exception
        else:
...
def requires_permission(required_permission: str):
    ...
    async def permission_dependency(
        ...
        if not has_permission:
            logger.warning(f"Permission denied for '{required_permission}'. User/Client: {current_user}")
            raise HTTPException(
