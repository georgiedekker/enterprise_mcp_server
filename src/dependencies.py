...
def get_db(request: Request) -> MCPPostgresDB:
    ...
    if db is None:
        raise RuntimeError("Database not found in app state. Lifespan might have failed.")
    return db
...
def get_auth_service(request: Request) -> 'AuthService':
    ...
    if auth_service is None:
        raise RuntimeError("AuthService not found in app state. Lifespan might have failed.")
    return auth_service
...
def get_audit_service(request: Request) -> AuditLogService:
    ...
    if audit_service is None:
        raise RuntimeError("AuditLogService not found in app state. Lifespan might have failed.")
...
def get_tool_registry(request: Request) -> FastMCP:
    ...
    if mcp_instance is None:
        raise RuntimeError("FastMCP instance not found in app state. Lifespan might have failed.")
    return mcp_instance
