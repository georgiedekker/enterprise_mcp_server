...
class MCPPostgresDB:
    ...
    def __init__(self, pool: asyncpg.Pool, loop: asyncio.AbstractEventLoop, executor: ThreadPoolExecutor):
        self.pool = pool
        self._loop = loop
    ...
    async def _create_tables(conn: asyncpg.Connection):
        ...
        await conn.execute('CREATE INDEX IF NOT EXISTS audit_logs_timestamp_idx ON audit_logs(timestamp);')
        await conn.execute('CREATE INDEX IF NOT EXISTS audit_logs_actor_id_idx ON audit_logs(actor_id);')
        await conn.execute('CREATE INDEX IF NOT EXISTS audit_logs_resource_type_resource_id_idx ON audit_logs(resource_type, resource_id);')
        
        # Create indexes for OAuth tables
        await conn.execute('CREATE INDEX IF NOT EXISTS oauth_clients_client_id_idx ON oauth_clients(client_id);')
        await conn.execute('CREATE INDEX IF NOT EXISTS oauth_auth_codes_code_idx ON oauth_auth_codes(code);')
        await conn.execute('CREATE INDEX IF NOT EXISTS oauth_auth_codes_expires_at_idx ON oauth_auth_codes(expires_at);')
        await conn.execute('CREATE INDEX IF NOT EXISTS oauth_access_tokens_token_idx ON oauth_access_tokens(token);')
        await conn.execute('CREATE INDEX IF NOT EXISTS oauth_access_tokens_expires_at_idx ON oauth_access_tokens(expires_at);')
        await conn.execute('CREATE INDEX IF NOT EXISTS oauth_refresh_tokens_token_idx ON oauth_refresh_tokens(token);')
        await conn.execute('CREATE INDEX IF NOT EXISTS oauth_refresh_tokens_expires_at_idx ON oauth_refresh_tokens(expires_at);')
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                if existing and replace_existing:
                    ...
                    logger.warning(f"Replacing existing tool '{name}' (UUID: {old_tool_uuid}). Associated versions and files will be deleted.")
                    await conn.execute('''
    ...
    async def add_multi_file_tool(self, name: str, description: str, entrypoint: str, files: Dict[str, str], created_by: Optional[int] = None, tool_dir_uuid: Optional[str] = None, replace_existing: bool = False) -> str:
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                if existing and replace_existing:
                    old_tool_uuid = existing['tool_id']
                    logger.warning(f"Replacing existing multi-file tool '{name}' (UUID: {old_tool_uuid}). Associated versions and files will be deleted.")
                    await conn.execute('''
                        DELETE FROM mcp_tool_versions WHERE tool_id = $1
                    ''', old_tool_uuid)
                    await conn.execute('''
    ...
    async def update_multi_file_tool(self, name: str, description: str, entrypoint: str, files: Dict[str, str]) -> bool:
    ...
    async def add_tool_version(
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                ''', tool_uuid)
                if is_multi:
                    raise ValueError("Versioning via add_tool_version is only supported for single-file tools.")
    ...
    async def log_audit_event(
        ...
        async with self.pool.acquire() as conn:
            log_id = await conn.fetchval('''
                ...
                RETURNING id
            ''', now, actor_id, actor_type, action_type, resource_type, resource_id, status, 
                json.dumps(details) if details else None, request_id, ip_address)
    ...
    async def create_user(self, username: str, password_hash: Optional[str] = None, api_key_hash: Optional[str] = None, email: Optional[str] = None) -> int:
        """Create a new user."""
        now = self._get_timestamp()
        try:
            async with self.pool.acquire() as conn:
    ...
    def __get_pydantic_json_schema__(cls, _core_schema, handler):
        ...
        """
        return {"type": "object", "title": "MCPPostgresDB", "description": "PostgreSQL database connection"}
    ...
    async def create_auth_code(
        ...
        async with self.pool.acquire() as conn:
            ...
            """,
            code, client_id, user_id, scope, code_challenge, 
            code_challenge_method, redirect_uri, expires_at, now
            )
    ...
    async def create_tokens(self, client_id: str, user_id: int, scope: Optional[str]) -> Dict[str, str]:
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                """,
                refresh_token, client_id, user_id, scope, access_token_id, refresh_expires, now
                )
        
        return {"access_token": access_token, "refresh_token": refresh_token}
    
    async def validate_refresh_token(self, refresh_token: str, client_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def validate_client_credentials(self, client_id: str, client_secret: str) -> Optional[Dict[str, Any]]:
    ...
    async def create_client_credentials_token(self, client_id: str, scope: Optional[str]) -> Dict[str, str]:
        """Create an access token for client credentials flow (no refresh token)."""
        logger.info(f"Creating client credentials token for client '{client_id}' with scope '{scope}'")
        ...
        async with self.pool.acquire() as conn:
            try:
                ...
                logger.debug(f"Created token {access_token[:15]}... for client '{client_id}' with scope: '{scope}'")
            except Exception as e:
    ...
    async def list_users(self) -> List[Dict[str, Any]]:
        ...
        async with self.pool.acquire() as conn:
            rows = await conn.fetch("SELECT id, username, is_active, created_at FROM users ORDER BY username")
            return [dict(row) for row in rows] 