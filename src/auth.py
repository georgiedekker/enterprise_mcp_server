...
def check_scope_permission(
    ...
    logger.debug(
        f"Checking if '{required_permission}' is granted by scopes: {granted_scopes}"
    )
    ...
    if "*:*" in granted_scopes or "*" in granted_scopes:
        logger.debug(
            f"Permission '{required_permission}' granted - wildcard match (*:* or *)"
        )
        return True
...
class AuthService:
    ...
    def verify_api_key(self, plain_api_key: str, hashed_api_key: str) -> bool:
        ...
        check_hash = hashlib.pbkdf2_hmac(
            "sha256", plain_api_key.encode("utf-8"), salt, 50000
        )
    ...
    async def initialize_roles_and_permissions(self):
        ...
        role_permissions = {
            ...
            "api_gateway": [
                "gateway:configure"
            ],  # Permissions needed by gateway itself
        }
    ...
    try:
        ...
        if payload.get("_jwt_decode_failed"):  # Check if JWT decode failed
            ...
            if username is None or user_id_str is None:
                logger.warning(
                    "Invalid JWT payload: missing username or sub (user ID)."
                )
                raise credentials_exception
            ...
            try:
                ...
                logger.warning(
                    f"Invalid JWT payload: sub '{user_id_str}' is not an integer."
                )
                raise credentials_exception
...
def requires_permission(required_permission: str):
    ...
    async def permission_dependency(
        ...
        logger.debug(
            f"Permission granted for '{required_permission}'. User/Client: {current_user}"
        )
