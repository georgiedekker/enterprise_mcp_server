...
async def restore_tool_version(
    ...
    try:
        ...
        version_to_restore = next(v for v in versions if v["version_number"] == restore_data.version_number)
        ...
        if restore_data.version_number == current_version:
            return {"message": f"Version {restore_data.version_number} is already the current version"}
